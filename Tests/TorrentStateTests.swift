...
struct TorrentStateTests {
    ...
    @Test("TorrentState all enum cases exist")
    func torrentStateAllEnumCases() {
        let allCases = TorrentState.allCases
        
        #expect(allCases.contains(.downloading))
        #expect(allCases.contains(.seeding))
        #expect(allCases.contains(.paused))
        #expect(allCases.contains(.checking))
    ...
    @Test("TorrentState CaseIterable conformance")
    func torrentStateCaseIterableConformance() {
        let allCases = TorrentState.allCases
        
        // Verify all expected cases are present
        let expectedStates: Set<TorrentState> = [
            .downloading, .seeding, .paused, .checking, .queued, .error
        ]
        let actualStates = Set(allCases)
        
        #expect(actualStates == expectedStates)
    }
    
    @Test("TorrentState allCases order consistency")
    func torrentStateAllCasesOrderConsistency() {
        // Test that allCases returns the same order consistently
        let firstCall = TorrentState.allCases
        let secondCall = TorrentState.allCases
        
        #expect(firstCall.count == secondCall.count)
        
        for (index, state) in firstCall.enumerated() {
            #expect(state == secondCall[index], "Order should be consistent at index \(index)")
        }
    }
    
    // MARK: - Edge Cases and Integration Tests
    
    @Test("TorrentState with Array operations")
    func torrentStateWithArrayOperations() {
        let states: [TorrentState] = [.downloading, .seeding, .paused, .downloading]
        
        // Test filtering
        let downloadingStates = states.filter { $0 == .downloading }
        #expect(downloadingStates.count == 2)
        
        // Test unique filtering
        let uniqueStates = Array(Set(states))
        #expect(uniqueStates.count == 3) // downloading, seeding, paused
    }
    
    @Test("TorrentState with Dictionary operations")
    func torrentStateWithDictionaryOperations() {
        let stateCounts: [TorrentState: Int] = [
            .downloading: 5,
            .seeding: 3,
            .paused: 2
        ]
        
        #expect(stateCounts[.downloading] == 5)
        #expect(stateCounts[.seeding] == 3)
        #expect(stateCounts[.paused] == 2)
        #expect(stateCounts[.error] == nil)
    }
    
    @Test("TorrentState string interpolation")
    func torrentStateStringInterpolation() {
        let state = TorrentState.downloading
        let description = "Current state: \(state)"
        
        #expect(description.contains("downloading"))
        #expect(description == "Current state: downloading")
    }
}