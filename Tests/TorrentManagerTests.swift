...
struct TorrentManagerTests {
    ...
    private func createTestEnvironment() -> (TorrentManager, MockSession, MockAppPreferences, MockTorrentClientActing) {
        ...
        mockPreferences.autoRefreshInterval = 2.0
        mockPreferences.sortOption = SortOption(property: .dateAdded)
        ...
        return (torrentManager, mockSession, mockPreferences, mockClient)
    }
    ...
    func torrentManagerInitializesWithEmptyTorrentsAndLabels() {
        let (torrentManager, _, _, _) = createTestEnvironment()
    ...
    func torrentManagerInitializesWithTimerBasedOnPreferences() {
        let (_, _, mockPreferences, _) = createTestEnvironment()
    ...
    func filteredTorrentsAppliesSearchQuery() async throws {
        let (torrentManager, _, mockPreferences, mockClient) = createTestEnvironment()
    ...
    func filteredTorrentsWithCombinedFilters() async throws {
        ...
        // Arrange
        let testTorrents = [
            TestDataFactory.createStandardTorrent(hash: "hash1", name: "Movie Download", state: .downloading, label: "movies"),
