...
struct TestUtilities {
    ...
    static func validateTorrentConsistency(
        ...
        let sourceLocation = SourceLocation(fileID: file.description, filePath: file.description, line: Int(line), column: 0)
        
        // Progress should be between 0 and 1
        #expect(torrent.progress >= 0 && torrent.progress <= 1, 
               "Progress should be between 0 and 1", 
               sourceLocation: sourceLocation)
        
        // Downloaded should not exceed size
        #expect(torrent.downloaded <= torrent.size, 
