...
struct ServerTests {
    ...
    func serverInitializationWithNilKeychainData() {
        // Arrange
        let name = "Test Server"
        let type = ServerType.qbittorrent
        let data = "test data".data(using: .utf8)!
    ...
    func serverDecodingFromJSON() throws {
        ...
        let decodedServer = try decoder.decode(Server.self, from: encodedData)
    ...
    func serverDecodingWithMissingOptionalFields() throws {
        ...
        // Act
        let server = try decoder.decode(Server.self, from: jsonData)
    ...
    func serverDecodingWithInvalidServerType() {
        ...
        #expect(throws: DecodingError.self) {
            _ = try decoder.decode(Server.self, from: jsonData)
        }
    }
    ...
    func serverInequalityWithDifferentProperties() {
        ...
        // Different name
        let differentNameServer = Server(
        ...
        // Different type
        let differentTypeServer = Server(
        ...
        // Different data
        let differentDataServer = Server(
            name: baseServer.name,
            type: baseServer.type,
        ...
        let differentKeychainServer = Server(
            name: baseServer.name,
            type: baseServer.type,
            data: baseServer.data,
    ...
    func serverHashingConsistency() {
        ...
        let serverSet: Set<Server> = [server1, server2]
    ...
    @Test("Server with Array operations")
    func serverWithArrayOperations() {
        let servers = [
            TestDataFactory.createServer(name: "Server 1", type: .deluge),
            TestDataFactory.createServer(name: "Server 2", type: .qbittorrent),
            TestDataFactory.createServer(name: "Server 3", type: .deluge)
        ]
        
        // Test filtering by type
        let delugeServers = servers.filter { $0.type == .deluge }
        #expect(delugeServers.count == 2)
        
        // Test finding by name
        let foundServer = servers.first { $0.name == "Server 2" }
        #expect(foundServer?.name == "Server 2")
        #expect(foundServer?.type == .qbittorrent)
    }
    
    @Test("Server with Dictionary operations")
    func serverWithDictionaryOperations() {
        let server1 = TestDataFactory.createServer(name: "Server 1")
        let server2 = TestDataFactory.createServer(name: "Server 2")
        
        let serverDict: [String: Server] = [
            server1.id: server1,
            server2.id: server2
        ]
        
        #expect(serverDict["Server 1"] == server1)
        #expect(serverDict["Server 2"] == server2)
        #expect(serverDict["Nonexistent"] == nil)
    }
    
    @Test("Server sorting")
    func serverSorting() {
        let servers = [
            TestDataFactory.createServer(name: "Charlie"),
            TestDataFactory.createServer(name: "Alpha"),
            TestDataFactory.createServer(name: "Bravo")
        ]
        
        let sortedServers = servers.sorted { $0.name < $1.name }
        
        #expect(sortedServers[0].name == "Alpha")
        #expect(sortedServers[1].name == "Bravo")
        #expect(sortedServers[2].name == "Charlie")
    }
}