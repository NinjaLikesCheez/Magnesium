...
struct TorrentMapperTests {
	...
	struct FilteringTests {
		...
		// TorrentMapper.filter is private; filtering is exercised indirectly through `map`,
		// which applies filtering before sorting.
		private static let noSort = TorrentSortOption(property: .name, direction: .ascending)
		...
		func filterTorrentsBySingleState() {
			...
			let filterOptions = TorrentFilterOptions(states: [.downloading], labels: [])

			// Act
			let result = TorrentMapper.map(torrents, query: "", sortOption: Self.noSort, filterOptions: filterOptions)
		...
		func filterTorrentsByMultipleStates() {
			...
			let filterOptions = TorrentFilterOptions(states: [.downloading, .seeding], labels: [])
		...
		func filterTorrentsBySingleLabel() {
			...
			let filterOptions = TorrentFilterOptions(states: [], labels: ["Movies"])
		...
		func filterTorrentsByMultipleLabels() {
			...
			let filterOptions = TorrentFilterOptions(states: [], labels: ["Movies", "Music"])
		...
		func searchFunctionalityWithBasicQuery() {
			...
			// Act
			let result = TorrentMapper.map(torrents, query: "linux", sortOption: Self.noSort, filterOptions: filterOptions)
		...
		func searchFunctionalityWithSpecialCharacters() {
			...
			let result1 = TorrentMapper.map(
				torrents, query: "movie title", sortOption: Self.noSort, filterOptions: filterOptions)
			let result2 = TorrentMapper.map(torrents, query: "tv show", sortOption: Self.noSort, filterOptions: filterOptions)
			let result3 = TorrentMapper.map(
				torrents, query: "game title", sortOption: Self.noSort, filterOptions: filterOptions)
		...
		func searchFunctionalityWithPartialMatches() {
			...
			// Act
			let result = TorrentMapper.map(torrents, query: "matrix", sortOption: Self.noSort, filterOptions: filterOptions)
		...
		func combinedFilteringStateAndLabel() {
			...
			let filterOptions = TorrentFilterOptions(states: [.downloading], labels: ["Movies"])
		...
		func combinedFilteringStateLabelandSearch() {
			...
			// Act
			let result = TorrentMapper.map(torrents, query: "action", sortOption: Self.noSort, filterOptions: filterOptions)
		...
		func filterWithEmptyTorrentList() {
			...
			// Act
			let result = TorrentMapper.map(torrents, query: "test", sortOption: Self.noSort, filterOptions: filterOptions)
		...
		func filterWithEmptyFiltersReturnsAllTorrents() {
			...
			// Assert
			#expect(result.count == 3)
			#expect(Set(result) == Set(torrents))
		}
		...
		func searchWithNoMatchesReturnsEmptyList() {
			...
			// Act
			let result = TorrentMapper.map(
				torrents, query: "nonexistent", sortOption: Self.noSort, filterOptions: filterOptions)
	...
	struct SortingTests {
		...
		func sortByNameAscendingCaseInsensitive() {
			...
			// Act
			let result = TorrentMapper.map(torrents, query: "", sortOption: sortOption, filterOptions: TorrentFilterOptions())
		...
		func sortByNameDescendingCaseInsensitive() {
			...
			let sortOption = TorrentSortOption(property: .name, direction: .descending)
		...
		func sortByDateAddedAscending() {
			...
			let sortOption = TorrentSortOption(property: .dateAdded, direction: .ascending)
		...
		func sortByDownloadSpeedAscending() {
			...
			let sortOption = TorrentSortOption(property: .downloadSpeed, direction: .ascending)
		...
		func sortByUploadSpeedAscending() {
			...
			let sortOption = TorrentSortOption(property: .uploadSpeed, direction: .ascending)
		...
		func sortByUploadSpeedDescending() {
			...
			let sortOption = TorrentSortOption(property: .uploadSpeed, direction: .descending)
		...
		func sortByProgressAscending() {
			...
			let sortOption = TorrentSortOption(property: .progress, direction: .ascending)
